function [L,C,SUMD,D] = fit(X, K, varargin)
%__________________________________________________________________________
% anatomy.segment.kmeans.fit
%--------------------------------------------------------------------------
% Learn a K-means clustering from observed [weighted] data.
%
% FORMAT [L,C,SUMD,D] = kmeans.fit(X,K,...)
% 
% MANDATORY
% ---------
...
import anatomy.segment.kmeans.apply

% -------------------------------------------------------------------------
% Parse inputs
p = inputParser;
p.FunctionName = 'kmeans.fit';
...
if numel(K) > 1
    [L,C] = apply(X, K, p.Results.Distance, p.Results.Missing);
    return
end
