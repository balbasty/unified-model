...
% gmm.fit instead.
%
% FORMAT [resp,cluster,prop,lb] = gmm.loop(obs,cluster,prop,...)
%
% MANDATORY
% ---------
%
...
if ~iscell(cluster) || numel(cluster) < 2
    error('[math.gmm.loop] At least one mean and one precision matrix are needed.');
else
    if ~iscell(cluster{1})
        MU = cluster{1};
    else
...
if Missing, cst = lib.const(mean, prec, L);
else,       cst = lib.const(mean, prec);
end
logpX = lib.marginal(X, [{MU} prec], cst, {C,L}, BinUncertainty);
...
for em=1:IterMax
    ...
    if sum(BinUncertainty) > 0
    	SS2b = lib.suffstat('bin', BinUncertainty, Z, W, {C,L});
    else
        SS2b = 0;
    end
    ...
    % . Cluster parameters (MU,b,A/V,n) are updated
        ...
        [lSS0,lSS1,lSS2] = lib.suffstat('base', X, Z, W, {C,L});
        
        % -----------------------------------------------------------------
        % Initialise objective function
        [LMU,LA]    = lib.kl.gausswishart({MU,b}, prec, {MU0,b0}, {V0,n0});
        [LX,cst]    = lib.marginalsum(lSS0, lSS1, lSS2, mean, prec, L, SS2b);
        LB          = NaN(1,SubIterMax);
        LB(1)       = LMU + LA + LX;
        for i=1:SubIterMax
            ...
            [SS0,SS1,SS2] = lib.suffstat('infer', lSS0, lSS1, lSS2, {MU,A}, L);
            SS2 = SS2 + SS2b;

            % -------------------------------------------------------------
            % Update GMM
            [MU,A,b,V,n] = lib.updateclusters( ...
    ...
    % Classical M-step
        ...
        [SS0,SS1,SS2] = lib.suffstat(X, Z, W);
        SS2 = SS2 + SS2b;
    ...
    % Update Proportions
    if size(PI,1) == 1
        [PI,logPI,a] = lib.updateproportions(SS0, a0);
    end        

    % ---------------------------------------------------------------------
    % Plot GMM
    if Verbose(1) >= 3
        plot.fit({X,W}, {MU,A}, PI);
    end
    ...
    % ---------------------------------------------------------------------
    % Compute lower bound
    lb.P(end+1) = lib.kl.dirichlet(a, a0);
    lb.Z(end+1) = lib.kl.categorical(Z, W, logPI);
    if ~Missing
        [lb.MU(end+1),lb.A(end+1)] = lib.kl.gausswishart(...
